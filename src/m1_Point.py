...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance = self.distance + math.sqrt((x-self.x)**2 + (
            y-self.y)**2)
        self.x = x
        self.y = y
    ...
    def get_number_of_moves_made(self):
        return self.moves
    def get_distance_from(self, anthorpoint):
        x1 = anthorpoint.x
        y1 = anthorpoint.y
        distance_from = math.sqrt((self.x - x1)**2 + (self.y-y1)**2)
    ...
    def closer_to(self,point2,point3):
        ...
        x3 = point3.x
        y3 = point3.y
        distance1 = math.sqrt((x2-self.x)**2+(y2-self.y)**2)
        distance2 = math.sqrt((x3-self.x)**2+(y3-self.y)**2)
